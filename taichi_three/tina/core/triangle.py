        ...
        for f in ti.smart(self.get_faces_range()):
            ...
            for P in ti.grouped(ti.ndrange((bot.x, top.x + 1), (bot.y, top.y + 1))):
                ...
                if all(wei >= 0):
                    ...
                    reached = wei.x * Al + wei.y * Bl + wei.z * Cl
                    xx = reached - self.engine.from_viewspace(V23(self.engine.from_viewport(pos), 0.))
                    depth_f = self.engine.to_viewspace(reached).z
                    depth = depth_f * self.engine.maxdepth
                    #print(Al, Av, depth_f)
