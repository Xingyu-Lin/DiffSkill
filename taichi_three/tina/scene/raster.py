...
class Scene:
    def __init__(self, res_x=512, res_y=None, **options):
        ...
        self.init_control(**self.options)
    ...
    def add_object(self, object, material=None, raster=None):
        ...
        elif raster == 'particle_sdf':
            self.particle_sdf_raster = tina.ParticleSDFRaster(self.engine, **self.options)
            raster = self.particle_sdf_raster

        self._ensure_material_shader(material)

        self.objects[object] = namespace(material=material, raster=raster)

    def init_control(self, gui=None, center=None, theta=None, phi=None, radius=None,
                     fov=60, is_ortho=False, blendish=True, **kwargs):
        '''
    ...
    def update_control(self, gui=None, center=None, theta=None, phi=None, radius=None,
                       fov=60, is_ortho=False, blendish=True, **kwargs):

        self.control.fov = fov
        self.control.is_ortho = is_ortho
        self.control.blendish = blendish
        if center is not None:
            self.control.center[:] = center
        self.control.init_rot(theta, phi)
    ...
    def render(self):
        ...
        for object, oinfo in self.objects.items():
            ...
            if hasattr(oinfo.raster, 'plb_set_particles'):
                oinfo.raster.plb_set_particles()
    ...
    def input(self, gui, refresh=False):
        '''
