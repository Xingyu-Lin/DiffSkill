...
class Loss:
    def __init__(self, cfg, sim: MPMSimulator):
        ...
        self.contact_weight = ti.field(dtype=dtype, shape=len(sim.primitives))  # Contact loss only applied the activated
        ...
        self.num_particle_target = 0
        self.particle_target = ti.Vector.field(self.dim, dtype=dtype, shape=np.prod(self.res))
        self.stepsize = min(self.n_particles // 1000, 1)
        self.enable_target_update = True
    ...
    def load_target_density(self, path=None, grids=None):
        if path is not None or grids is not None:
            ...
            idxes = (np.array(np.where(grids > 1e-4)).transpose() * self.dx).astype(grids.dtype)
            self.num_particle_target = idxes.shape[0]
            idxes = np.vstack([idxes, np.zeros(shape=(np.prod(self.res) - self.num_particle_target, self.dim), dtype=idxes.dtype)])

            self.particle_target.from_numpy(idxes)  # Threshold for finding target particle

    def initialize(self):
        self.sdf_weight[None] = self.cfg.weight.sdf
        self.density_weight[None] = self.cfg.weight.density
        for i in range(self.contact_weight.shape[0]):
            self.contact_weight[i] = self.cfg.weight.contact
        self.chamfer_weight[None] = 0.
        self.soft_contact_loss = self.cfg.soft_contact

        target_path = self.cfg.target_path
        self.load_target_density(target_path)

    def set_weights(self, sdf, density, contact, is_soft_contact, chamfer):
        self.sdf_weight[None] = sdf
        self.density_weight[None] = density
        for i in range(self.contact_weight.shape[0]):
            self.contact_weight[i] = contact
        self.chamfer_weight[None] = chamfer
    ...
    def set_target_update(self, flag):
        self.enable_target_update = flag

    def update_target(self):
        self.target_sdf_copy.fill(self.inf)
        if self.enable_target_update:
    ...
    def compute_contact_loss_kernel(self):
        for i in ti.static(range(len(self.primitives))):
            self.contact_loss += self.contact_weight[i] * (self.primitives[i].min_dist[None] ** 2)
    ...
    def compute_chamfer_loss_kernel(self, f: ti.i32):
        ...
        num_particle = self.n_particles // self.stepsize
        for i in range(num_particle):
            min_dist = 1e6
            for j in range(self.num_particle_target):
                d = ti.sqrt((self.particle_x[f, i * self.stepsize][0] - self.particle_target[j][0]) ** 2 +
                            (self.particle_x[f, i * self.stepsize][1] - self.particle_target[j][1]) ** 2 +
                            (self.particle_x[f, i * self.stepsize][2] - self.particle_target[j][2]) ** 2)
                if d < min_dist:
                    min_dist = d
            self.chamfer_loss[None] += min_dist / num_particle

        for i in range(self.num_particle_target):
            min_dist = 1e6
            for j in range(num_particle):
                d = ti.sqrt((self.particle_x[f, j][0] - self.particle_target[i * self.stepsize][0]) ** 2 +
                            (self.particle_x[f, j][1] - self.particle_target[i * self.stepsize][1]) ** 2 +
                            (self.particle_x[f, j][2] - self.particle_target[i * self.stepsize][2]) ** 2)
                if d < min_dist:
                    min_dist = d
            self.chamfer_loss[None] += min_dist / self.num_particle_target
    ...
    def sum_up_loss_kernel(self):
        self.loss[None] += self.contact_loss[None]
    ...
    # For recovering the exact state for CEM
    def get_state(self):
        loss_state = {
            '_start_loss': self._start_loss,
            '_last_loss': self._last_loss,
            '_init_iou': self._init_iou
        }
        return loss_state

    def set_state(self, _start_loss, _last_loss, _init_iou):
        self._start_loss = _start_loss
        self._last_loss = _last_loss
        self._init_iou = _init_iou
