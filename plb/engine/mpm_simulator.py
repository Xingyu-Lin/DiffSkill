...
class MPMSimulator:
    def __init__(self, cfg, primitives=()):
        ...
        quality = cfg.quality * cfg.quality_multiplier
        if self.dim == 3:
            quality = quality * 0.5
        self.n_particles = ti.field(ti.uint32, shape=())
        ...
        # Collision
        self.num_particle_collision = 600  # Number of particles for collision check
        self.set_num_collision()  # Read collision pairs from yaml
        if self.num_collision > 0:
            self.rand_num = ti.Vector.field(dim, dtype=dtype,
                                            shape=(self.num_collision,
                                                   self.num_particle_collision))  # Generating random particles from [-1, 1] using fixed random seed
            self.rand_points = ti.Vector.field(dim, dtype=dtype,
                                               shape=(max_steps, self.num_collision, self.num_particle_collision))  # Random points on the surface
            self.collision_idx = ti.field(ti.i32, shape=(max_steps, self.num_collision))

    def set_num_collision(self):
        num_collision = 0
        # self.collision_table = np.ones_like([self.n_primitive, self.n_primitive], dtype=np.int) * -1# Map from primitive idx to collidion id
        self.collision_pairs = []
        for i in range(self.n_primitive):
            for j in range(self.n_primitive):
                if self.primitives[i].cfg.collision_group[j] > 0:
                    # self.collision_table[i, j] = num_collision
                    self.collision_pairs.append([i, j])
                    num_collision += 1
        self.num_collision = num_collision
        return num_collision

    def initialize(self, n_particles):
        ...
        self.set_num_collision()
        if self.num_collision > 0:
            prev_state = np.random.get_state()
            np.random.seed(42)
            rand_num = np.random.uniform(-1.5, 1.5, (
                self.num_collision, self.num_particle_collision,
                self.dim))  # Larger random numbers to encourage points to be on the corners and edges
            self.rand_num.from_numpy(rand_num)
            # self.set_rand_num_kernel(rand_num)
    ...
    def g2p(self, f: ti.i32):
        for p in range(0, self.n_particles[None]):  # grid to particle (G2P)
            ...
            self.x[f + 1, p] = ti.max(ti.min(self.x[f, p] + self.dt * self.v[f + 1, p], 1. - 3 * self.dx), self.lower_bound * self.dx)
            # advection and preventing it from overflow

    @ti.kernel
    def set_surface_points(self, s: ti.int32):  # TODO may have issues with multiple collision due to the global data access rule
        for cid in ti.static(range(self.num_collision)):
            i, j = ti.static(self.collision_pairs[cid])
            for k in range(self.num_particle_collision):
                rand_point = self.rand_num[cid, k] * self.primitives[j].size[None]  # TODO Only works for box for now (Currently the box)
                self.rand_points[s + 1, cid, k] = self.primitives[j].get_surface_pos(s + 1, rand_point)

    @ti.kernel
    def set_collision_idx(self, s: ti.int32):
        for cid in ti.static(range(self.num_collision)):
            i, j = ti.static(self.collision_pairs[cid])
            self.collision_idx[s + 1, cid] = self.primitives[i].get_collision_idx(s + 1, cid, self.rand_points)

    @ti.kernel
    def apply_collision_projection(self, s: ti.int32):
        for cid in ti.static(range(self.num_collision)):
            i, j = ti.static(self.collision_pairs[cid])
            if self.collision_idx[s + 1, cid] != -1:
                self.primitives[i].collision_projection(s + 1, self.rand_points[s + 1, cid, self.collision_idx[s + 1, cid]])
    ...
    def substep(self, s):
        ...
        if self.num_collision > 0:
            self.set_surface_points(s)
            self.set_collision_idx(s)
            self.apply_collision_projection(s)
    ...
    def substep_grad(self, s):
        ...
        self.grid_op.grad(s)
        if self.num_collision > 0:
            self.apply_collision_projection.grad(s)
            self.set_surface_points.grad(s)
    ...
    def get_state(self, f):
        print('mpm simulator get state', self.n_particles[None])
        x = np.zeros((self.n_particles[None], self.dim), dtype=np.float64)  # Pos
        v = np.zeros((self.n_particles[None], self.dim), dtype=np.float64)  # Vel
        F = np.zeros((self.n_particles[None], self.dim, self.dim), dtype=np.float64)  # Deformation grad
        C = np.zeros((self.n_particles[None], self.dim, self.dim), dtype=np.float64)
    ...
    def set_primitive_state(self, f, state):
        for s, i in zip(state[4:], self.primitives):
            i.set_state(f, s)
    ...
    def get_x(self, f):
        x = np.zeros((self.n_particles[None], self.dim), dtype=np.float64)
    ...
    def get_v(self, f):
        v = np.zeros((self.n_particles[None], self.dim), dtype=np.float64)
    ...
    @ti.kernel
    def _get_torch_x(self, f: ti.int32, x: ti.ext_arr()):
        for i in range(self.n_particles[None]):
            for j in ti.static(range(self.dim)):
                x[i, j] = self.x[f, i][j]

    def get_torch_x(self, f, device):
        x = torch.zeros(size=(self.n_particles[None], self.dim), device=device)
        self._get_torch_x(f, x)
        return x
