...
def solve_cem(env, args):
    ...
    population_size = args.timestep_per_decision // args.max_iters
    if args.use_mpc:
        population_size = population_size // args.plan_horizon
    num_elites = population_size // 10

    print('env_horizon:', env.horizon)
    ...
    for idx in range(env.horizon):
        ...
        incremental_iou = info['incremental_iou']
        print('current iou:', info['iou'])
        print('incremental iou:', incremental_iou)
        img = env.render(mode='rgb_array')

        imgs.append(img)
        cv2.imwrite(os.path.join(logger.get_dir(), "{:0>4}.png".format(idx)), img[..., ::-1])
        sum_reward += reward
    save_numpy_as_gif(np.array(imgs), os.path.join(logger.get_dir(), "{}.gif".format(args.env_name)))
    # traj_dict = {
    #     'initial_state': initial_state,
    #     'action_traj': action_traj
    # }
    logger.record_tabular('incremental_iou', incremental_iou)
